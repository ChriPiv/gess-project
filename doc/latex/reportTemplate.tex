...
\begin{equation}
\phi(\tilde{X}, \tilde{X}_i) = e^{-||\tilde{X}-\tilde{X}_i||^2}
\end{equation}

In practice, $X$ must be first rescaled, because the different agent parameters are orders of magnitude different. At each desired point $\tilde{X}$, this is the classical weighted least squares problem
\begin{align}
